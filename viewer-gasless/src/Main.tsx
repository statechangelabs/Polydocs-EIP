...
const useSign = (contractAddress: string, tokenId: string) => {
  ...
  const sign = useCallback(async () => {
    ...
    const contract = Registry__factory.connect(contractAddress, provider);
    const termsUrl = await contract.templateUrl(tokenId);
      ...
      try {
        ...
        if (typeof token === "undefined") {
          ...
          const term = isNaN(blockTag)
            ? await contract.term(token, realKey)
            : await contract.term(token, realKey, {
                blockTag,
              });
          ...
          const events = await contract.queryFilter(
            contract.filters.TermChanged(token),
            0,
            blockTag
          );
...
const Renderer: FC<{
  ...
  useAsyncEffect(async () => {
    ...
    setURI(strippedUri);
    const events = await contract.queryFilter(
      contract.filters.TemplateChanged(tokenId),
      0,
      parseInt(block)
    );
    const lastEvent = events.pop()?.blockNumber;
